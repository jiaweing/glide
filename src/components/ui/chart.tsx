      ...
      >
        ...
        <div className="grid gap-1.5">
          {payload.map((item, index) => {
            ...
            const indicatorColor = color ?? item.payload?.fill ?? item.color ?? "#000000";
...
>(({ className, hideIcon = false, payload, verticalAlign = "bottom", nameKey }, ref) => {
  ...
  const getItemKey = (item: PayloadItem, index: number): string => {
...
function getPayloadConfigFromPayload(config: ChartConfig, payload: unknown, key: string) {
  ...
  if (key in payload && typeof (payload as Record<string, unknown>)[key] === "string") {
    configLabelKey = (payload as Record<string, string>)[key] ?? configLabelKey;
  } else if (
    payloadPayload &&
    key in payloadPayload &&
    typeof (payloadPayload as Record<string, unknown>)[key] === "string"
  ) {
    configLabelKey = (payloadPayload as Record<string, string>)[key] ?? configLabelKey;
  }
