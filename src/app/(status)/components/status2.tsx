...
export default function Status2({ user }: { user: { email: string } }) {
          ...
          {locations.map((location) => {
            ...
            return (
              <Marker
                key={location.name}
                position={position}
                icon={isPickup ? pickupIcon : isDropoff ? dropoffIcon : selectableIcon}
                eventHandlers={{
                  click: () => {
                    if (!pickupLocation) {
                      handleLocationSelect(location.name, "pickup");
                    } else if (!dropoffLocation) {
                      handleLocationSelect(location.name, "dropoff");
                    } else {
                      // Allow changing existing selection
                      if (isPickup) {
                        ...
                        // Select as new pickup if pickup is cleared, otherwise as dropoff
                        if (!pickupLocation) {
                          handleLocationSelect(location.name, "pickup");
                        } else {
                          handleLocationSelect(location.name, "dropoff");
      ...
      {/* <div className="absolute left-0 right-0 top-0 z-10 m-2 space-y-4 rounded-xl bg-white p-4">
          ...
          <div className="flex items-center space-x-2">
            ...
            <div className="flex w-full flex-col space-y-1">
              ...
              <div className="text-sm font-medium">
                {pickupLocation || "Click a pin on the map to select pickup location"}
              </div>
            </div>
          </div>
        )}
        <div className="flex items-center space-x-2">
          ...
          <div className="flex w-full flex-col space-y-1">
            ...
            <div className="text-sm font-medium">
              {dropoffLocation || "Click a pin on the map to select dropoff location"}
            </div>
          </div>
        </div>
